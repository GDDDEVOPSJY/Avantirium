...
use Yii;
use common\models\Qualificationform;
use common\models\QualificationformSearch;
 ...
 */
class QualificationformController extends Controller
{
    /**
     * @inheritdoc
     */
    ...
    public function beforeAction($action) {           
        ...
        if(($action->id=='index' || $action->id=='view') && !in_array("qualificationform_view", $permissions)){
            throw new \yii\web\ForbiddenHttpException('You have no permission to access this section');
        }
        if($action->id=='create' && !in_array("qualificationform_add", $permissions)){
            throw new \yii\web\ForbiddenHttpException('You have no permission to access this section');
        }
        if($action->id=='update' && !in_array("qualificationform_edit", $permissions)){
            throw new \yii\web\ForbiddenHttpException('You have no permission to access this section');
        }
        if($action->id=='delete' && !in_array("qualificationform_delete", $permissions)){
            throw new \yii\web\ForbiddenHttpException('You have no permission to access this section');
        }   
    ...
    {
        $searchModel = new QualificationformSearch();
    ...
    {
        if (($model = Qualificationform::findOne($id)) !== null) {
            return $model;
        } else {
